...
def main():
    ...
    model = LLM(model=args.model,
            ...
            dtype='half')
    prompt = f"Translate the following {language[args.src_lang]} sentence split in lines into {language[args.tgt_lang]} and explain it in detail (the translation should be split in lines too):\n" + "{text}" + f"<{args.tgt_lang}>:"

    identifiers = list(data.keys())
    source_sentences = [SEP.join(data[id]['text']) for id in identifiers]
    translations = forward(model, source_sentences, prompt)

    for id, translation in zip(identifiers, translations):
        data[id]['text'] = translation.split(SEP)
